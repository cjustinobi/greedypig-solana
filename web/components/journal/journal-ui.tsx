...
export function JournalCreate() {
  const { createEntry, joinGame } = useJournalProgram();
...
function JournalCard({ account }: { account: PublicKey }) {
   const { joinGame, accounts } = useJournalProgram()
  const { accountQuery } = useJournalProgramAccount({
    account,
  });
  const { publicKey } = useWallet();
  const [message, setMessage] = useState("");
  ...
  const isFormValid = message.trim() !== "";
    ...
    const handleJoinGame = (gameAccount: any) => {
      if (publicKey) {
